...
// 生成示例高级分析数据
function generateSampleAdvancedData(nodes: KnowledgeNode[], progress: ProgressMap) {
  ...
  nodes.forEach((node) => {
    ...
    const questionAttemptStats: Record<CognitiveLevel, { total: number; correct: number; avgTimeSpent: number; guessRate: number }> = {} as any;
...
export function LearningAnalyticsPage({ nodes, edges = [], progress, recommendations, onSelect }: Props) {
  ...
  // 生成高级分析数据
  const advancedData = useMemo(() => generateSampleAdvancedData(nodes, progress), [nodes, progress]);
                ...
                <div className="chart-card">
                  <ProgressDistributionChart
                    statusCounts={[
                      ...
                      { status: "weak", count: weak, label: "需巩固", color: "#f59e0b" },
                      { status: "not_started", count: nodes.length - mastered - learning - weak, label: "未开始", color: "#94a3b8" },
                    ]}
                  />
                </div>
